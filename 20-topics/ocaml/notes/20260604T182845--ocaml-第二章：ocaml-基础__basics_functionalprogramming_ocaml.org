...
#+date:       [2026-06-04 Thu 18:28]
#+startup: overview
#+filetags: :basics:functionalprogramming:ocaml:
#+identifier: 20260604T182845
...
这可以展示类型推导。因为 ~y ~ 0~ ，所以 ~y~ 是 ~int~ 。又因为 ~if~ 分支里有 ~1~ ，所以整个 ~if~ 返回 ~int~（[[denote:20260604T182845::#if-的语义][可以参考这里]]）。因为使用 ~*~ ，所以参与运算的 输入的 ~x~ 也必须是 ~int~ ；所以函数类型是 ~int= 。
...
其中的 ~-~ 就表示 [[denote:20260601T181248::#小练习][没有名字]] 。这个函数的意思是：“接收 ~x~ ，返回 ~x + 1~ 的函数。
...
它们语法不同，但语义等价。[[denote:20260706T153131::#lambda-与-let-的关系][参见这里]] 。
...
~'a~ 是类型变量，表示未知类型。[[denote:20260604T182845::#函数定义的语义][可以参考]] 。CS3110 解释说，类型变量总以单引号开头，常见有 ~'a~、~'b~、~'c~ 。因为 ~id~ 可以作用于很多类型，所以它是多态函数（polymorphic function）。
...
我们之前把它理解为是“接受两个int，返回一个int”。但是CS3110进一步指出，[[denote:20260706T153131::#多参函数与-lambda-演算][函数类型是右结合的]]:
...
*以上过程就是 [[denote:20260601T180530][部分应用(partial application)]]*。函数没有一次性拿到全部参数，也可以先拿到一部分参数，返回一个等待剩余参数的新函数。我们还可以进一步观看这个函数:
...
- 这个东西是 expression 还是 definition ? [[denote:20260604T182845::#expression][expression]]
...
它是一个[[denote:20260604T182845::#递归与非递归函数][递归函数定义]]； ~rec~ 让函数名在函数体内部可见； ~if~ 是[[denote:20260604T182845::#if][表达式]]； guard ~n = 0~ 的类型是 ~bool~ ； then 分支 ~1~ 是 ~int~ ； else 分支 ~n * fact (n - 1)~ 也是 ~int~ ；所以[[denote:20260604T182845::#函数定义的语义][整个函数]]返回 ~int~ ；由于用了 ~*~ 和 ~-~ ，所以 ~n~ 也被推断为 ~int~ ；因此函数类型就是 ~int -> int~ ；它对负数不终止，所以文档中要写 ~Requires:[n >= 0]~ ；它不是[[denote:20260604T182845::#尾递归][尾递归]]，所以大输入可能栈溢出；即便改成尾递归，也可能整数溢出。
